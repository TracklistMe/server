...
module.exports.controller = function(app) {
  ...
  /**
   * GET /auth/TWitter
   * Login with Twitter (works in GET, not in POST)
   */
          ...
          model.User.find({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a Twitter account belonging to you'
              });
            }
            ...
            model.User.find({
              ...
              if (!user) {
                return res.status(400).send({
                  message: 'Something went wrong in the linkage process'
                });
              }
              ...
              user.save().success(function() {
                res.send({
                  token: authenticationUtils.createToken(user)
                });
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          model.User.findOne({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a Google account belonging to you'
              });
            }
            var token = req.headers.authorization.split(' ')[1];
            var payload = jwt.decode(token, config.TOKEN_SECRET);
          ...
          model.User.findOne({
            ...
            if (existingUser) {
              return res.send({
                token: authenticationUtils.createToken(existingUser)
              });
            }
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          model.User.findOne({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a GitHub account belonging to you'
              });
            }
            var token = req.headers.authorization.split(' ')[1];
            var payload = jwt.decode(token, config.TOKEN_SECRET);
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          model.User.findOne({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a LinkedIn account belonging to you'
              });
            }
            var token = req.headers.authorization.split(' ')[1];
            var payload = jwt.decode(token, config.TOKEN_SECRET);
      ...
      function(profile) {
        if (!req.headers.authorization) {
          ...
          User.findOne({
            ...
            if (user) {
              return res.status(409).send({
                message: 'There is already a Live account belonging to you'
              });
            }
            var token = req.headers.authorization.split(' ')[1];
            var payload = jwt.decode(token, config.TOKEN_SECRET);
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          User.findOne({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a Facebook account belonging to you'
              });
            }
            var token = req.headers.authorization.split(' ')[1];
            var payload = jwt.decode(token, config.TOKEN_SECRET);
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          User.findOne({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a Yahoo account belonging to you'
              });
            }
            var token = req.headers.authorization.split(' ')[1];
            var payload = jwt.decode(token, config.TOKEN_SECRET);
      ...
      request.get({
        ...
        if (req.headers.authorization) {
          User.findOne({
            ...
            if (existingUser) {
              return res.status(409).send({
                message: 'There is already a Foursquare ' +
