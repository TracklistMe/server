...
function validateFile(xmlPath) {
    var deferred = Q.defer();
    var filename = "temporaryFileName"
    var image = cloudstorage.createReadStream(xmlPath)
    var writeFile = fs.createWriteStream(filename);




    image.pipe(writeFile)
    ...
    /*
    var streamPointer = cloudstorage.createWriteStream(xmlPath)
    ...
    /*
    console.log('TEST' + streamPointer.read())
    console.log(stream)

    var xml = new xmlStream(stream);
    console.log(xml)
    console.log("collect tracks")
    xml.collect('tracks');
